...
// cosmic: optional setting to unlock wallet for block minting only;
...
// cosmic: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // cosmic: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // cosmic: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // cosmic: not avatar mode
                        {
...
// cosmic: create coin stake transaction
...
// cosmic: check 'spent' consistency between wallet and txindex
// cosmic: fix wallet spent state according to txindex
...
// cosmic: disable transaction (only for coinstake)
